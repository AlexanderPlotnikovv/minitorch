...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        one_minus_sig = sig.f.add_zip(sig.f.neg_map(sig), sig._ensure_tensor(1.0))
        derivative = sig.f.mul_zip(sig, one_minus_sig)
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        ...
        list_order = [int(order[i]) for i in range(order.size)]
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        list_order = [
            k[0] for k in sorted(enumerate([int(order[i]) for i in range(order.size)]), key=lambda x: x[1])
        ]
        return grad_output._new(grad_output._tensor.permute(*list_order)), 0.0
